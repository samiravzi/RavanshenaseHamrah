...
public class PersianDateParser {
	...
	public PersianCalendar getMyPersianDate() {

		checkDateStringInitialValidation();

		ArrayList<String> tokens = splitMyDateString(normalizeDateString(dateString));
		...
		int day = Integer.parseInt(tokens.get(3));
		
		
		Date date;
		try {
			date = new SimpleDateFormat("MMM", Locale.ENGLISH).parse(tokens.get(4));
			
			Calendar cal = Calendar.getInstance();
		    cal.setTime(date);
		    int month = cal.get(Calendar.MONTH);
			
			String temp[] = tokens.get(1).split(":");
			...
			PersianCalendar pCal = new PersianCalendar();
			pCal.set(year, month, day, hour, min);
	...
	private ArrayList<String> splitMyDateString(String dateString) {
		ArrayList<String> tokens= new ArrayList<String>();
	
		StringTokenizer st = new StringTokenizer(dateString);
		int i = 0;
		//Log.e("samiraaaaaaa","---- Split by space ------");
		while (st.hasMoreElements()) {
			tokens.add(st.nextElement().toString()); 
			//Log.e("token "+i, tokens.get(i)+"");
			i++;
		}
	//	if (tokens.size() != 3)
		//	throw new RuntimeException("wrong date:" + dateString + " is not a Persian Date or can not be parsed");
